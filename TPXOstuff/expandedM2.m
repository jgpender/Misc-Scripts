...
idxs = find(tpxo.lon>=model.lon(1)&tpxo.lon<=model.lon(end));
jdxs = find(tpxo.lat>=model.lat(1)&tpxo.lat<=model.lat(end));
imagesc(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.uamp(jdxs,idxs)/100);caxis([0,.2]);axis xy;rect;hold on
               contour(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.uamp(jdxs,idxs)/100,[0:.5:1],'LineColor','Black','Showtext','on')
